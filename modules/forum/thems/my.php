...
*/
$title = 'Мои темы';
$menu = '<a href="/" style="color: white;">Главная</a> | <a href="/forum" style="color: white;">Форум</a> | Мои темы';
$where = 'forum';
include_once($_SERVER["DOCUMENT_ROOT"].'/design/head.php');
mode('user');
?>
<div class="menu2">
	Всего <b><?=$db->query("SELECT `id` FROM `forum_t` WHERE `id_author`=".$user['id']."")->num_rows?></b> тем
</div>
<?
$Pagination = new Pagination("SELECT * FROM `forum_t` WHERE `id_author`=".$user['id']." ORDER BY `id`", $_GET['page']);
$query = $db->query("SELECT * FROM `forum_t` WHERE `id_author`=".$user['id']." ORDER BY `id` DESC LIMIT ".$Pagination->start.", ".$Pagination->end."");
