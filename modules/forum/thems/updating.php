...
*/
$title = 'Обновлённые темы';
$menu = '<a href="/" style="color: white;">Главная</a> | <a href="/forum" style="color: white;">Форум</a> | Обновлённые темы';
$where = 'forum';
...
<?
$Pagination = new Pagination("SELECT * FROM `forum_t`", $_GET['page']);
$query = $db->query("SELECT * FROM `forum_t` ORDER BY `last` DESC LIMIT ".$Pagination->start.", ".$Pagination->end."");
...
$Pagination->out('/forum/thems/updating/');
