...
if(isset($_POST['yes'])){
	$db->query("INSERT INTO `forum_rating_post` SET `id_us`=".$user['id'].", `id_post`=".$_GET['id'].", `time`=".time().", `type`='1', `id_author`=".$post['id_us']."");
	$db->query("UPDATE `users` SET `rating`=`rating`+0.01 WHERE `id`=".$post['id_us']."");
	$db->query("INSERT INTO `notifications` SET `id_us`=".$post['id_us'].", `text`='us{".$user['id']."} положительно оценил ваш пост:[br][b]".mb_substr($post['msg'], 0, 100, 'UTF-8')."[/b] в теме [url=/forum/thema".$post['id_thema']."/page".lstPage($post['id_thema'], 1)."]".$Forum->nameThema($post['id_thema'])."[/url]', `time`=".time().", `section`=2");
	header('location:/forum/thema'.$post['id_thema'].'/page'.lstPage($post['id_thema'], 1));
}
?>
<div class="list1">
	Вы уверены в том что хотите поставить <b><font color="green">плюс</font></b>
